...
class PyGitlabList(GitlabBaseAction):
    ...
    def run(self, **kwargs):
        ...
        LOG.debug(f"Gitlab Module : {api_module}")
        module_id = kwargs.pop(module_id, None)
        LOG.debug(f"Gitlab Module ID : {module_id}")
        api_submodule = kwargs.pop(api_submodule, None)
        LOG.debug(f"Gitlab SubModule : {api_submodule}")
        submodule_id = kwargs.pop(submodule_id, None)
        LOG.debug(f"Gitlab Submodule ID : {submodule_id}")
        ...
        module = getattr(pyglab, api_module)
